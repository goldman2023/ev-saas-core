    ...
    public function pay()
    {
        ...
        try {
            ...
            */
            ...
            $this->order->invoicing_start_date = Carbon::now()->toDateTimeString(); // start invoicing this moment (for non-trial subscriptions)
            ...
            $order_item->title = method_exists($this->item, 'hasMain') && $this->item->hasMain() ? $this->item->main->name : $this->item->name; // TODO: Think about changing Product `name` col to `title`, it's more universal!
            ...
            $invoice->due_date = $default_due_date; // Default due_date is 7 days from now (if not trial mode, if it's trial it's )
            ...
            if(!Auth::check()) {
                $new_user = User::where('email', $this->order->email)->first();

                if(!empty($new_user->id ?? null) && Hash::check($this->account_password, $new_user->password)) { // check password again just in case
            ...
            return redirect()->route('checkout.order.received', $this->order);
        } catch(\Exception $e) {
            DB::rollBack();
        }
    }

    protected function executePayment(Request $request, $invoice_id) {
