...
if (! function_exists('castValueForSave')) {
    function castValueForSave($key, $setting, $data_types)
    {
        ...
        if ($data_type === Upload::class || $data_type === Category::class) {
        ...
        } elseif ($data_type === Currency::class) {
            ...
            $value = $value ? 1 : 0;
        } elseif ($data_type === 'array' && $data_type === 'uploads' && is_array($data_type)) {
            ...
            foreach ($settings as $key => $setting) {
                ...
                try {
                    ...
                    if (isset($settings[$key]) && ! empty($value)) {
                        ...
                        } elseif ($data_type === Category::class) {
                            $settings[$key]['value'] = Category::find($value);
                        } elseif ($data_type === 'uploads') {
                            $uploads = [];
    ...
    function areActiveRoutes(array $routes, $output = 'active')
    {
        foreach ($routes as $route) {
            ...
            if (Route::currentRouteName() == 'attributes.edit') {
                ...
                if ($route == 'product.attributes.edit' && $attribute->content_type == Product::class) {
                    return $output;
                }
                if ($route == 'seller.attributes.edit' && $attribute->content_type == 'App\Models\Seller') {
                    return $output;
                }
            }
        }
