...
class CategoryUtility
{
    ...
    public static function get_immediate_children($id, $with_trashed = false, $as_array = false)
    {
        ...
        $children = $as_array && ! is_null($children) ? $children->toArray() : [];
    ...
    {
        $children = self::get_immediate_children($id, $with_trashed, true);
    ...
    public static function flat_children($id, $with_trashed = false, $container = [])
    {
        ...
        if (! empty($children)) {
            foreach ($children as $child) {
                $container[] = $child;
                $container = self::flat_children($child['id'], $with_trashed, $container);
            }
        }
    ...
    {
        $children = self::flat_children($id, $with_trashed = false);
    ...
    {
        $children_ids = self::get_immediate_children_ids($id, true);

        $category = Category::where('id', $id)->first();

        self::move_level_up($id);
    ...
    {
        if (self::get_immediate_children_ids($id, true) > 0) {
            foreach (self::get_immediate_children_ids($id, true) as $value) {
                $category = Category::find($value);
                $category->level -= 1;
                $category->save();

                return self::move_level_up($value);
            }
        }
    }
    ...
    public static function move_level_down($id)
    {
        if (self::get_immediate_children_ids($id, true) > 0) {
            foreach (self::get_immediate_children_ids($id, true) as $value) {
                ...
                return self::move_level_down($value);
            }
        }
    }
    ...
    public static function delete_category($id)
    {
        $category = Category::where('id', $id)->first();
        if (! is_null($category)) {
            self::move_children_to_parent($category->id);
            $category->delete();
        }
    }
}
