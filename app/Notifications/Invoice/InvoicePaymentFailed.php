    ...
    public function toMail($notifiable)
    {
        try {
            return (new WeMailMessage)
                ->markdown('vendor.notifications.invoice.invoice-payment-failed', ['invoice' => $this->invoice, 'user' => $notifiable])
                ->subject(translate('Payment failed for invoice on '.get_tenant_setting('site_name')));
        } catch(\Exception $e) {
