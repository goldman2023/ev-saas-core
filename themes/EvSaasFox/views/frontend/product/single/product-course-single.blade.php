        ...
        <div class="w-full bg-white rounded-xl shadow overflow-y-auto">
            <div class="w-full pb-8">
                <img class="w-full h-full max-h-[460px] object-cover"
                    src="{{ !empty($product->cover) ? $product->getCover(['w' => 0, 'h' => 600]) : $product->getTHumbnail(['w' => 0, 'h' => 600]) }}"
                    alt="" />
            </div>
    ...
    <div class="col-span-12 md:col-span-4">
        ...
        <div class="w-full flex flex-col bg-white rounded-xl shadow p-5 mb-5">
            ...
            <div class="w-full flex items-center mb-3">
                ...
                <span x-data="{}" class="badge-success px-2 py-2 ml-2 !text-14 items-center !font-semibold"
                    :class="{ 'flex': base_price !== total_price, '!hidden': base_price === total_price }">
                    @svg('heroicon-s-tag', ['class' => 'w-4 h-4 mr-1'])
                    <span
                        x-text="'{{ translate('%x%%') }}'.replace('%x%', (100-(100*total_price/base_price)).toFixed(0) )"></span>
                </span>
            </div>
            ...
            <div class="pb-3 w-full">
                ...
                @if(!$course_items->isNotEmpty())
                    @if(auth()->user()->manages($product) ?? false)
                        <a class="text-sm" target="_blank" href="{{ route('product.edit.course', $product->id) }}">
                            {{ translate('Add your course chapters!') }}
                        </a>
                    @endif
                @endif
                @endauth
