...
export default class Uploader {
   ...
   */
  uploadSelectedFile({ onPreview , imageContainer }) {
    ...
    // default uploading
      ...
      let image_id = imageContainer.getAttribute('image-id');
      let imageIndex = Array.from(imageContainer.parentNode.children).indexOf(imageContainer);

      Livewire.find(livewire_form_id).emit('showMediaLibrary', 'editor-js-promise', 'image', [{}], image_id)

      window.addEventListener("we-media-selected-event", (event) => {
        if(event.detail.for_id === 'editor-js-promise' && event.detail.editorjs_media_wrapper_id === image_id && event.detail.selected.length > 0) {
          upload = (new Promise((resolve, reject) => {
            ...
            this.onUpload(response, imageIndex);
          }).catch((error) => {
            this.onError(error);
          });
        }
