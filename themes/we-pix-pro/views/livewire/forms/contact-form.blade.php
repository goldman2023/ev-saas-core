<form action="#" method="POST" class="relative mt-6 grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6" x-data="{
    ...
    <div class="sm:col-span-2">
        ...
        <div class="mt-1">
            <textarea wire:model.defer="message" rows="4" class="@error('message') is-invalid @enderror py-3 px-4 block w-full shadow-sm text-gray-900 border border-gray-300 rounded-md"></textarea>
    ...
    <div class="sm:col-span-2">
        <div class="flex items-start">
          <div class="flex-shrink-0">
            <button type="button" @click="consent = !consent" :class="{'bg-primary':consent,'bg-gray-200':!consent}" class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary" role="switch" aria-checked="false">
              <span class="sr-only">Agree to policies</span>
              <span aria-hidden="true" :class="{'translate-x-5':consent,'translate-x-0':!consent}" class="inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"></span>
            </button>
          </div>
          <div class="ml-3">
            <p class="text-base text-gray-500 @error('consent') text-danger @enderror cursor-pointer" @click="consent = !consent" >
              {{ translate('I have read and accept the') }}
              <a href="#" class="font-medium text-primary underline">{{ translate('Privacy Policy') }}</a>
            </p>
          </div>
        </div>
