...
<div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5 sm:mt-5" x-data="{}">
    ...
    <div class="mt-1 sm:mt-0 sm:col-span-2">
        <div class="w-full" x-data="{
            ...
            count() {
                if(settings.pix_pro_dataset_samples === undefined || settings.pix_pro_dataset_samples === null) {
                    settings.pix_pro_dataset_samples = [{...this.template}];
                }
        
                return settings.pix_pro_dataset_samples.length;
            },
            add() {
                if(this.count() >= this.limit) {
                    return;
                ...
                settings.pix_pro_dataset_samples.push({...this.template});
            },
            remove(index) {
                settings.pix_pro_dataset_samples.splice(index, 1);
            },
         }"
        >
            <template x-if="count() >= 1">
                <template x-for="(item, index) in settings.pix_pro_dataset_samples">
                    <div class="w-full flex">
                        <div class="grid grid-cols-12 gap-3" :class="{'mt-2': index > 0}">
                            <input type="text" 
                                    class="form-standard col-span-12 lg:col-span-3"
                                    x-bind:placeholder="'{{ translate('Title') }}'"
                                    x-model="item.title" />
    
                                <input type="text" 
                                    class="form-standard col-span-12 lg:col-span-3"
                                    x-bind:placeholder="'{{ translate('Description') }}'"
                                    x-model="item.description" />
    
                                <input type="text" 
                                    class="form-standard col-span-12 lg:col-span-3"
                                    x-bind:placeholder="'{{ translate('File Size') }}'"
                                    x-model="item.file_size" />
