...
<x-livewire-tables::table.cell class="align-middle static ">
    <div class="flex static justify-center" role="group" x-data="{ isOpen: false }" x-cloak>
        ...
        {{-- show actions ony if: 1) subscription is active OR license expiration_date is in future --}}
        @if(
            $row->user_subscription->first()->isActive()
            ||
            (!empty($row->getData('expiration_date')) && $row->getData('expiration_date') > now())
        )
            <ul x-show="isOpen"
