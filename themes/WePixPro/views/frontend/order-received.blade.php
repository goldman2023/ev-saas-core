...
<section class="bg-white">
    ...
    <div class="max-w-3xl mx-auto px-4 py-8 sm:px-6 sm:py-16 lg:px-8">
        <div class="w-full mb-3">
            ...
            <h1 class="text-sm font-semibold uppercase tracking-wide text-info">{{ translate('In checkout process') }}
            </h1>
            @else
            ...
            <p class="mt-2 text-base text-gray-500 mb-4">{{ str_replace('%d%', $order->id, 'Your order #%d% has been
                abandoned. You can always continue with purchase by clicking the button below.') }}</p>
            ...
            <p class="mt-2 text-base text-gray-500 mb-4">{{ str_replace('%d%', $order->id, 'Your order #%d% is either
                waiting for checkout to be finished or is currently being processed by payment processor. Please
                continue with checkout process or wait until the payment is realized.') }}</p>
            @elseif($first_item instanceof \App\Models\Plan)
            <p class="mt-2 text-4xl font-extrabold tracking-tight sm:text-5xl">{{ translate('Successfully bought a
                plan!') }}</p>
            <p class="mt-2 text-base text-gray-500 mb-4">{{ str_replace('%d%', $order->id, 'Your order #%d% has been
                processed. You have successfully subscribed to plan listed below.') }}</p>

            @if(!empty($ghost_user))
            {{-- User is a guest/ghost user --}}
            <x-system.alert type="warning"
                text="{!! translate('Please finalize your registration.').'<br>'.translate('Email').': '.$ghost_user->email !!}"
                :only-text="true" />
            @else
            ...
            <p class="mt-2 text-base text-gray-500 mb-4">{{ str_replace('%d%', $order->id, 'Your order #%d% has been
                processed. Please select the available booking time.') }}</p>
            ...
            <p class="mb-2 text-4xl font-extrabold tracking-tight sm:text-5xl">{{ translate('We are processing the
                order!') }}</p>
            <p class="mb-2 text-base text-gray-500">{{ str_replace('%d%', $order->id, 'Your order #%d% has been received
                and will be processed asap.') }}</p>
            ...
            <p class="mb-2 text-4xl font-extrabold tracking-tight sm:text-5xl">{{ translate('Successfully bought a
                course!') }}</p>
            <p class="mb-2 text-base text-gray-500">{{ str_replace('%d%', $order->id, 'Your order #%d% has been
                processed. You successfully bought a course.') }}</p>


            <div class="w-full mb-4">
                @if(!empty($ghost_user))
                <x-system.alert type="warning"
                    text="{!! translate('Before accessing the course, please go to your email and finalize your registration.').' <br /> '.translate('Email').': '.$ghost_user->email !!}"
                    :only-text="true" />
                @else
        ...
        <div class="border-t border-gray-200">
                            ...
                            <dl class="flex text-sm divide-x divide-gray-200 space-x-4 sm:space-x-6">
                                ...
                                <div class="pl-4 flex sm:pl-6">
                                    ...
                                    <dd class="ml-2 text-gray-700">
                                        {{ FX::formatPrice($item->total_price) }} {{ $order->type === 'subscription' ? '
                                        / ' . $order->invoicing_period : '' }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-3">
                @if(!$order->is_temp)
                <div
                    class="col-span-3 md:col-span-1 py-4 md:py-6 space-y-0 space-x-3 md:space-x-0 md:space-y-3 border-b border-gray-200 md:border-none">
                    {{-- @if(!empty($order->invoices->first()?->meta['test_stripe_hosted_invoice_url'] ?? null)) --}}
                    @if($order->invoices->first())
                    <a href="{{ route('invoice.download', $order->invoices->first()->id) }}" target="_blank"
                    ...
                    <a href="{{ route('stripe.portal_session') }}" target="_blank"
                        class="btn-primary min-w-[130px] justify-center">
                    ...
                    <div class="w-full flex flex-col">
                        ...
                        <div class="w-full flex justify-between mb-1">
                            <span class="text-14 text-gray-600 font-normal">
                                {{ $order_item->subject->name }}: {{ $order_item->quantity.' '.translate('user') }} x {{
                                \FX::formatPrice($order_item->total_price) }} / {{ translate('user') }} / {{
                                $order->invoicing_period }}
                            </span>

                            <span class="text-14 text-gray-600 font-normal">{{ \FX::formatPrice($order_item->quantity *
                                $order_item->total_price) }} / {{ $order->invoicing_period }}</span>
                        </div>
                    ...
                    <div class="w-full flex flex-col mt-3 pt-3 border-t border-gray-200">
                        ...
                        <div class="w-full flex justify-between items-center mt-3 pt-3 border-t border-gray-200">
                            <span class="text-14 text-gray-600 font-normal">
                                ...
                                <strong>
                                    {{-- @dd($order->load(['invoices' => fn($query) =>
                                    $query->withoutGlobalScopes()])->invoices) --}}
                                    {{ \Carbon::createFromTimestamp($order->load(['invoices' => fn($query) =>
                                    $query->withoutGlobalScopes()])->invoices->first()?->end_date)->format('d M, Y') }}
                                </strong>
                            </span>

                            <span class="text-14 text-gray-600 font-normal">
                                {{ translate('Total') }}: <strong>{{ $subscription->getTotalPrice(format: true) }} / {{
                                    $order->invoicing_period }}</strong>
                            </span>
                        </div>
                    </div>
                    @else
