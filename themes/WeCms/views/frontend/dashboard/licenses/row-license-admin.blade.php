    ...
    <div class="flex static justify-center" role="group" x-data="{ isOpen: false }" x-cloak>
        ...
        >
            ...
            @if(auth()->user()?->isAdmin() ?? false)
                ...
                <li class="border-t border-gray-200">
                    <button type="button"
                        class="w-full flex items-center px-3 py-3 pr-4 text-gray-900 text-14 hover:bg-primary hover:text-white"
                        @click="$dispatch('display-modal', {'id': 'pix-pro-edit-license', 'license_id': '{{ $row->id ?? '' }}'})">
                ...
                <li class="border-t border-gray-200">
                    <button type="button"
                        ...
                        wire:click="removeLicense({{ $row->id }})">
