{{-- TODO: This will be refactored split better, but it does the job for now --}}
	...
	<div>
		...
		<div class="fixed inset-0 flex z-40 lg:hidden" role="dialog" aria-modal="true">
			...
			<div class="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl py-4 pb-12 flex flex-col overflow-y-auto">
				<div class="px-4 flex items-center justify-between">
					...
					<button type="button" class="-mr-2 w-10 h-10 bg-white p-2 rounded-md flex items-center justify-center text-gray-400"> <span class="sr-only">Close menu</span>
				...
				<form class="mt-4 border-t border-gray-200">
					...
					<ul role="list" class="font-medium text-gray-900 px-2 py-3">
						<li> <a href="#" class="block px-2 py-3">
                                    Totes
                                </a> </li>
						<li> <a href="#" class="block px-2 py-3">
                                    Backpacks
		...
		<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
			<div class="relative z-10 flex items-baseline justify-between pt-24 pb-6 border-b border-gray-200">
				<h1 class="text-4xl font-extrabold tracking-tight text-gray-900 lg:flex">

                        <!--   TODO: Create some dynamic component to resolve all titles in case of different scenarions (Category/Content type/ etc)                      -->
                        @if (isset($brand_id))

                            @php
                                $title = \App\Models\Brand::find($brand_id)->name;
                                $description = \App\Models\Brand::find($brand_id)->meta_description;
                            @endphp
                        @else
                            @php
                                $title = 'Products';
				...
				<div class="flex items-center">
					<div class="relative inline-block text-left">
						<div>
							<button type="button" class="group inline-flex justify-center text-sm font-medium text-gray-700 hover:text-gray-900" id="menu-button" aria-expanded="false" aria-haspopup="true"> Sort
							...
							<div class="py-1" role="none">
								<!--
                                    ...
                                    --><a href="#" class="font-medium text-gray-900 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                        Most Popular
                                    </a> <a href="#" class="text-gray-500 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-1">
                                        Best Rating
                                    </a> <a href="#" class="text-gray-500 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-2">
                                        Newest
                                    </a> <a href="#" class="text-gray-500 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-3">
                                        Price: Low to High
                                    </a> <a href="#" class="text-gray-500 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-4">
                                        Price: High to Low
                                    </a> </div>
						</div>
					</div>
					<button type="button" class="p-2 -m-2 ml-5 sm:ml-7 text-gray-400 hover:text-gray-500"> <span class="sr-only">View grid</span>
					...
					<button type="button" class="p-2 -m-2 ml-4 sm:ml-6 text-gray-400 hover:text-gray-500 lg:hidden"> <span class="sr-only">Filters</span>
						<!-- Heroicon name: solid/filter -->
						<svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
							<path fill-rule="evenodd" d="M3 3a1 1 0 011-1h12a1 1 0 011 1v3a1 1 0 01-.293.707L12 11.414V15a1 1 0 01-.293.707l-2 2A1 1 0 018 17v-5.586L3.293 6.707A1 1 0 013 6V3z" clip-rule="evenodd" /> </svg>
					</button>
				</div>
			</div>
			<section aria-labelledby="products-heading" class="pt-6 pb-24">
				<h2 id="products-heading" class="sr-only">dadas</h2>
				<div class="grid grid-cols-1 lg:grid-cols-4 gap-x-8 gap-y-10">
					<!-- Filters -->
					<form class="hidden lg:block">
						<x-tenant.system.category.category-list-basic></x-tenant.system.category.category-list-basic>
						<x-tenant.system.filtering.category-filters></x-tenant.system.filtering.category-filters>
					</form>
					<!-- Product grid -->
					<div class="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:col-span-3 lg:gap-x-8"> @if($content == 'product' || $content == null) @foreach ($products as $key => $product)
						<x-tenant.product.cards.image-overlay-and-button :product="$product"></x-tenant.product.cards.image-overlay-and-button> @endforeach @endif {{-- TODO: Check if pagination wotks propertly --}} @if ($content == 'product' || isset($category_id))
						<div class="aiz-pagination aiz-pagination-center d-flex justify-content-center mt-4"> {{ $products->links() }} </div> @endif </div>
				</div>
			</section>
		</main>
	</div>
