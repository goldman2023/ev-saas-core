...
@section('content')
<div class="bg-gray-50 pb-9">
    <div class="container ">
        <div class="px-6 pb-12">
            <x-theme::dashboard.orders.customer-order-steps :order="$order" class="mb-6 pt-[60px] static md:static">
            </x-theme::dashboard.orders.customer-order-steps>
        </div>
    </div>
</div>
...
<section class="bg-gray-50 pb-12">
    ...
    <div class="card max-w-5xl mx-auto px-4 py-8 sm:px-6 sm:py-9 lg:px-8 mb-9">
        <div class="w-full mb-3">
            <h1 class="text-sm font-semibold uppercase tracking-wide text-primary">{{ translate('Thank you!') }}</h1>

            @if($order->isAbandoned())
            <p class="mt-2 text-4xl font-bold sm:text-5xl">{{ translate('Abandoned order!') }}</p>
            ...
            <p class="mt-2 text-4xl font-bold  sm:text-5xl">{{ translate('Please review your order') }}</p>
            ...
            <p class="mb-2 text-4xl font-bold sm:text-5xl">{{ translate('We are processing the order!') }}</p>
                    ...
                    <div class="flex-auto flex flex-col">
                        <div>
                            ...
                            <ul class="w-full flex flex-col gap-y-2 mt-2">
                                ...
                                <li
                                    class="w-full flex items-center justify-between border border-gray-200 rounded px-2 py-1">
                                    <div class="flex items-center ">
                                        <span class="pr-2">+</span>
                                        <strong class="text-12 line-clamp-1 pr-2">{{ $addon_item->name }}</strong>
                                        <span class="text-12 line-clamp-1">{{ translate('Quantity:') }} {{
                                            $addon_item->quantity }}</span>
                                    </div>

                                    <strong class="text-12 text-gray-900">{{ \FX::formatPrice($addon_item->total_price)
                                        }}</strong>
                                </li>
                                @endforeach
                            </ul>
                        ...
                        <div class="mt-6 flex-1 flex items-end">
                            <dl class="flex text-sm divide-x divide-gray-200 space-x-4 sm:space-x-6">
                                ...
                                @endif
                                <div
                                    class="@if($order->type === 'standard' || $order->type === 'installments') pl-4 flex sm:pl-6 @endif">
                                ...
                                <div class="pl-4 flex sm:pl-6">
                                    ...
                                    <dd class="ml-2 text-gray-700">
                                        {{ FX::formatPrice($item->total_price + $addons_total_sum) }} {{ $order->type
                                        === 'subscription' ? ' / ' . $order->invoicing_period : '' }}</dd>
                                    @else
                                    <dd class="ml-2 text-gray-700">
            ...
            <div class="grid grid-cols-3">
                ...
                <dl class="space-y-6 border-t border-gray-200 text-sm pt-6 col-start-1 md:col-start-2 col-end-4">
                    ...
                    <div class="flex justify-between">
                        ...
                        <dd class="text-gray-700">{{ $order->tax_incl ? '('.\FX::formatPrice($order->tax_amount).')' :
                            \FX::formatPrice($order->tax_amount) }}</dd>
                    </div>
                    @endif
