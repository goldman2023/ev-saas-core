<div class="bg-white border border-gray-200 rounded-lg" x-data="{
    ...
    <div class="px-3 py-3 mb-2 border-b border-gray-200">
        <div class="flex justify-between items-center flex-wrap sm:flex-nowrap">
            <div class="w-full">
                <h4 class="font-semibold">{{ translate('Bulk Actions') }}</h4>
            </div>
        </div>
    </div>
    <div class="px-3 py-3 sm:px-3" wire:loading.class="opacity-40 pointer-events-none" wire:target="runAction">
        <div class="flex flex-col">
            <div class="grid grid-cols-3 mb-5 items-center">
                <label class="block text-sm font-medium text-gray-900 dark:text-gray-400">
                    {{ translate('Action') }}
                </label>
                <select class="block col-span-2 p-2  w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    x-model="action">
            ...
            <button type="button" @click="runAction(event)" class="relative bg-gray-100 mb-2 p-3 flex items-center justify-between space-x-4 rounded-xl hover:bg-gray-200">
                <div>
                    <h3 class="text-sm font-medium text-gray-900 text-left">
