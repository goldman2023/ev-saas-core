<div class="card mb-9 !p-4 !pt-2">
            ...
            <li class="mr-2" role="presentation">
                <button class="inline-block font-medium text-lg p-3 border-b-2 rounded-t-lg"
                    ...
                    @if($order->tasks->where('type', 'delivery')->count() > 0)
                        <span class="ml-2 badge-success">{{ \Str::headline($order->tasks->where('type', 'delivery')->first()->status) }}</span>
                    @endif
                </button>
            </li>
            <li class="mr-2" role="presentation">
    ...
    <div id="order-queues-tabs-content">
        {{-- Delivery Queues --}}
        <div class="hidden rounded-lg bg-gray-50 dark:bg-gray-800" id="order-queues-tab-delivery-content" role="tabpanel">
            <div class="w-full flex flex-col">
                @if(!empty($deliveryTask))
                    <ul role="list" class="divide-y divide-gray-200 pb-1">
                        <li>
                            <a href="{{ route('task.edit', $deliveryTask->id) }}" target="_blank" class="block hover:bg-gray-50">
                                <div class="py-2">
                                    <div class="flex items-center justify-between">
                                        <p class="truncate text-sm font-medium text-indigo-600">{{ $deliveryTask->name }}</p>
                                    </div>
                                    <div class="mt-2 flex flex-column sm:flex-row sm:items-center">
                                        ...
                                        <p class="flex items-center text-sm text-gray-500 ml-2">
                                            ...
                                            {{ translate('Added to queue on: ') }} {{ $deliveryTask->created_at->format('d M, Y H:i') }}
                                        </p>
                                    </div>
                                </div>
                            </a>
        ...
        {{-- Printing Queues --}}
        <div class="hidden rounded-lg bg-gray-50 dark:bg-gray-800" id="order-queues-tab-printing-content" role="tabpanel">
            <div class="w-full">
