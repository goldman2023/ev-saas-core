<a href="{{ $blogPost->getPermalink() }}" class="block bg-white rounded-xl border border-gray-200 hover:shadow-lg pb-4">
    <div class="w-full">
        <div class="w-full aspect-square inline-flex items-center justify-center rounded-t-xl mb-5 overflow-hidden">
        ...
        <div class="w-full text-left px-5">
            @if(!empty($blogPost->primary_category))
                <div class="flex flex-row mb-2">
                    <span class="badge-primary !rounded-xl">{{ $blogPost->primary_category->name }}</span>
                </div>
            @endif
