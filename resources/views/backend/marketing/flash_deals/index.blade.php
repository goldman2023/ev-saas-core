            ...
            <tbody>
                @foreach($flash_deals as $key => $flash_deal)
                    <tr>
						...
						<td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('admin.flash_deals.edit', ['id'=>$flash_deal->id, 'lang'=>config('app.locale')] )}}" title="{{ translate('Edit') }}">
    ...
    <script type="text/javascript">
        function update_flash_deal_status(el){
            ...
            $.post('{{ route('admin.flash_deals.update_status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Flash Deal status updated successfully') }}');
                }
                else{
        ...
        function update_flash_deal_feature(el){
            ...
            $.post('{{ route('admin.flash_deals.update_featured') }}', {_token:'{{ csrf_token() }}', id:el.value, featured:featured}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Flash Deal featured successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
