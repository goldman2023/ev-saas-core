            ...
            <tbody>
                @foreach($pickup_points as $key => $pickup_point)
                    <tr>
						...
						<td class="text-right">
							<a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('admin.pick_up_points.edit', ['id'=>$pickup_point->id, 'lang'=>config('app.locale')] )}}" title="{{ translate('Edit') }}">
