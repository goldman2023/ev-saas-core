...
@invoke-delete.window="
    ...
    show = true;"
@delete-modal-hide.window="show = false;"
x-show="show"
x-cloak>
  <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    ...
    <div class="z-[9999] relative inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6"
      ...
      <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
        <button type="button" class="btn-danger sm:col-start-2 !justify-center sm:text-sm" @click="deleteModel()">{{ translate('Trash') }}</button>
        <button @click="show = false" type="button" class="mt-3 btn btn-white !justify-center sm:mt-0 sm:col-start-1 sm:text-sm">{{ translate('Cancel') }}</button>
      </div>
