<div>
    @if(!$disabled_registration)
        ...
        @endphp
        <form class="space-y-4 max-w-xl md:space-y-6 xl:max-w-2xl" wire:submit.prevent="register()" x-data="{
            entity: @js($entity),
            ...
            <div class="w-full" wire:loading.class="opacity-30 pointer-events-none">
                    ...
                    <fieldset class="mt-4">
                        <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                            <div class="flex items-center">
                                <input id="entity_individual" name="entity_field" selected type="radio" x-model="entity"
                                    value="individual"
                            ...
                            <div class="flex items-center">
                                <input id="entity_company" name="entity_field" type="radio" x-model="entity" value="company"
                                    class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                                <label for="entity_company" class="ml-3 block text-sm font-medium text-gray-700"> {{
                ...
                <div class="sm:grid-cols-2 grid gap-3">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            <input type="text" class="form-standard @error('surname') is-invalid @enderror"
                                placeholder="Smith" wire:model.defer="surname" data-test="we-register-surname">
                ...
                <div class="mb-3">
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="email" name="email"
                            class="form-standard @if($is_ghost) form-standard opacity-50 pointer-events-none @endif @error('email') is-invalid @enderror"
                            placeholder="you@example.com" wire:model.defer="email" data-test="we-register-email"
                            @if($is_ghost) readonly @endif>
                ...
                <div class="mb-3">
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="text" name="phone" class="form-standard @error('phone') is-invalid @enderror"
                            wire:model.defer="phone" data-test="we-register-phone-number">
                ...
                @if(collect(get_tenant_setting('user_meta_fields_in_use'))->where('registration', true)->count() > 0)
                @foreach(collect(get_tenant_setting('user_meta_fields_in_use'))->where('registration',
                true)->sortByOrderProperty() as $key => $options)
                <div class="mb-4" @if(in_array($key, \App\Models\UserMeta::metaForCompanyEntity()))
                    x-show="entity === 'company'" @endif>
                    <label class="block text-16 font-medium text-gray-700" @if($key==='address_state' )
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        ...
                        <div x-data="{
                            ...
                            wire:ignore.self --}} {{-- wire:key="{{ $company_vat_field_key }}"
                            ...
                            <div class="mt-1 flex" :class="{'opacity-50 pointer-events-none': !user_meta.address_country}">
                                ...
                                <button type="button" @click="checkVATvalidity()"
                                    :disabled="user_meta.address_country ? false : true"
                            ...
                            {{-- <p class="mt-2 text-sm text-red-600" id="email-error">Your password must be less than 4
                                characters.</p> --}}
                        </div>
                        @elseif($key === 'address_state')
                        <x-dashboard.form.select field="user_meta.{{ $key }}" selected="user_meta.{{ $key }}" :items="[]"
                            :search="true" :nullable="false"
                        ...
                        <x-dashboard.form.select field="user_meta.{{ $key }}" selected="user_meta.{{ $key }}"
                            :items="\Countries::getCodesForSelect(as_array: true)" :search="true" :nullable="false" />
                        @elseif(($options['type']??'string') == 'select')
                        <x-dashboard.form.select field="user_meta.{{ $key }}" selected="user_meta.{{ $key }}"
                            :items="\App\Models\UserMeta::metaSelectValues($key)" />
                        @endif
                    </div>
                </div>
                @endforeach
                ...
                <div class="mb-2 flex items-center justify-between">
                    <div class="w-full relative flex items-start ml-1">
                        ...
                        <div class="ml-3 text-sm">
                            <label for="register-form-terms-consent" class=font-medium text-gray-700 inline
                                @error('terms_consent') text-danger @enderror">
                                <span>
                ...
                {{-- Register Action --}}
                <div class="mb-3">
                    <button type="submit" class="btn bg-primary text-white w-full mt-2" @click="
    ...
    @else
        <p>{{ translate('Sorry, user registration is not enabled.') }}</p>
    @endif
</div>
