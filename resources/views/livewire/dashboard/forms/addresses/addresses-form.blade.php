<div>
    ...
    <div @if(!empty($componentId)) id="{{ $componentId }}" @endif
        class="p-4 border bg-white border-gray-200 rounded-lg shadow mt-5" x-data="{
        ...
        <div class="w-full flex flex-wrap justify-between items-center border-b border-gray-200 mb-3 pb-3">
            <div class="shrink-0">
                ...
                <p class="flex items-center-1 max-w-2xl text-sm text-gray-500"> {{ translate('Add one or multiple
                    addresses to your account') }}</p>
            </div>
            <div class="grow-0">
                <button type="button" class="btn-primary" wire:click="initNewAddress()"
                    @click="$dispatch('display-flyout-panel', {'id': 'address-panel'});">
        ...
        <div class="mt-6 sm:mt-5 space-y-6 sm:space-y-5 pb-2">
                    ...
                    <div class="w-full flex items-center justify-between p-6 space-x-6">
                        <div class="flex-1 truncate">
                            <div class="flex items-center space-x-3">
                                <h3 class="text-gray-900 text-sm font-medium truncate">{{ $address->address }}, {{
                                    $address->city }}</h3>
                            ...
                            <p class="mt-1 text-gray-500 text-sm truncate">{{ \Countries::get(code:
                                $address->country)->name ?? translate('Unknown') }}, {{ $address->zip_code }}</p>
                        </div>
                    </div>
                    <div>
                        <div class="-mt-px flex divide-x divide-gray-200">
                            <div class="w-0 flex-1 flex">
                                <button wire:click="removeAddress({{ $address->id }})"
                                    class="relative -mr-px w-0 flex-1 inline-flex items-center justify-center py-3 text-sm bg-danger text-white font-medium border border-transparent rounded-bl-lg">
                                    @svg('heroicon-o-trash', ['class' => 'w-4 h-4 text-white'])
                                    <span class="ml-3">{{ translate('Remove') }}</span>
                                </button>
                            </div>
                            <div class="-ml-px w-0 flex-1 flex">
                                <button wire:click="changeCurrentAddress({{ $key }})"
                                    class="relative w-0 flex-1 inline-flex items-center justify-center py-3 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500">
            ...
            @else
            <x-dashboard.empty-states.no-items-in-collection icon="heroicon-o-home"
                title="{{ translate('No addresses yet') }}"
                text="{{ translate('Add address(es) for easier checkout!') }}" {{-- link-href-route="plan.create" --}}
        ...
        <x-panels.flyout-panel id="address-panel">
            ...
            <div class="flex flex-col mb-1 grow">
                ...
                <div class="w-full">
                    ...
                    <div class="mt-1">
                        <input type="text" class="form-standard @error('currentAddress.address') is-invalid @enderror"
                            wire:model.defer="currentAddress.address">
                ...
                <div class="grid grid-cols-4 gap-3 mt-3">
                    ...
                    <div class="col-span-4 sm:col-span-2">
                        <label
                            class="block text-sm font-medium text-gray-700 @error('currentAddress.country') is-invalid @enderror">
                            {{ translate('Country') }}
                        </label>
                        <div class="mt-1">
                            <x-dashboard.form.select field="currentAddress.country"
                                :items="\Countries::getAll()->keyBy('code')->map(fn($item) => $item->name)"
                                selected="currentAddressCountry" :search="true" :nullable="false">
                    ...
                    <div class="col-span-4 sm:col-span-2">
                        ...
                        <div class="mt-1">
                            <input type="text" class="form-standard @error('currentAddress.state') is-invalid @enderror"
                                wire:model.defer="currentAddress.state">
                ...
                <div class="grid grid-cols-4 gap-3 mt-3">
                    ...
                    <div class="col-span-4 sm:col-span-2">
                        ...
                        <div class="mt-1">
                            <input type="text" class="form-standard @error('currentAddress.city') is-invalid @enderror"
                                wire:model.defer="currentAddress.city">
                    ...
                    <div class="col-span-4 sm:col-span-2">
                        ...
                        <div class="mt-1">
                            <input type="text"
                                class="form-standard @error('currentAddress.zip_code') is-invalid @enderror"
                                wire:model.defer="currentAddress.zip_code">
                ...
                <div class="w-full mt-4">
                    ...
                    <div class="mt-1">
                        <x-dashboard.form.text-repeater field="currentAddressPhones" limit="3"
                ...
                <div class="flex items-center mt-5">
                    <button type="button" @click="currentAddress_is_primary = !currentAddress_is_primary"
                        ...
                        role="switch">
                        <span
                            :class="{'translate-x-5':currentAddress_is_primary, 'translate-x-0':!currentAddress_is_primary}"
                            class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"></span>
                    </button>
                    <span class="ml-3" @click="currentAddress_is_primary = !currentAddress_is_primary">
                        <span class="text-sm font-medium text-gray-900 cursor-pointer">{{ translate('Is this your
                ...
                <div class="flex items-center mt-3">
                    <button type="button" @click="currentAddress_is_billing = !currentAddress_is_billing"
                        ...
                        role="switch">
                        <span
                            :class="{'translate-x-5':currentAddress_is_billing, 'translate-x-0':!currentAddress_is_billing}"
                            class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"></span>
                    </button>
