    ...
    <x-ev.alert id="successMessageContent" class="{{ !$insert_success ? 'd-none':'' }}"
        ...
        <div class="d-flex align-items-center mt-3">
            ...
            @if(!empty($product->id) && $insert_success)
                <a class="btn btn-white btn-sm mr-3" href="{{ route('product.edit', $product->id) }}" target="_parent">{{ translate('Edit') }}</a>
            @endif
        </div>
    </x-ev.alert>
...
{{--    @endif--}}
    ...
    <div class="card mb-3 mb-lg-5">
            ...
            <div class="card-header">
                <a href="{{ $product->id ? route('product.details', $product->id) : route('products.index') }}" class="text-secondary mr-3" style="height: 24px;">
