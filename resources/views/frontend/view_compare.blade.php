                        ...
                        <table class="table table-responsive table-bordered mb-0">
                                ...
                                <tr>
                                    ...
                                    @foreach (Session::get('compare') as $key => $item)
                                        <th scope="col" style="width:28%" class="font-weight-bold">
                                            <a class="text-reset fs-15" href="{{ \App\Models\Product::find($item)->getPermalink() }}">{{ \App\Models\Product::find($item)->getTranslation('name') }}</a>
                                        </th>
                                    @endforeach
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @foreach (Session::get('compare') as $key => $item)
                                        <td>{{ single_price(\App\Models\Product::find($item)->unit_price) }}</td>
                                    @endforeach
                                </tr>
                                <tr>
