...
<div class="col-span-12">
    ...
    <section class="relative w-full mb-3 md:mb-5 bg-white rounded-b-lg">
        <div class="relative w-full">
            <x-tenant.system.image class="block h-[180px] sm:h-[260px] w-full rounded-t-xl " fit="cover"
                :image="$user->getCover(['w' => 1200])" />
            <x-tenant.system.image
                class="absolute left-[30px] bottom-[-50px] w-[120px] h-[120px] sm:w-[160px] sm:h-[160px] rounded-full bg-white border border-gray-200"
                fit="contain" :image="$user->getThumbnail()" />
            @auth
            ...
            <div class="col-span-7 hidden md:flex flex-col">
                <ul class="w-full flex ">
                    ...
                    <div class="flex flex-col items-center py-3 px-3 text-16 text-typ-2 cursor-pointer">
                        ...
                        <strong class="text-typ-2">{{ $user->blog_posts()->where('type', 'portfolio')->count()
                            }}</strong>
                    </div>
                    <div class="flex flex-col items-center py-3 px-3 text-16 text-typ-2 cursor-pointer">
    ...
    <section class="grid grid-cols-12 gap-6 max-w-2xl md:max-w-full mx-auto">
        <div class="col-span-12 md:col-span-5 flex flex-col space-y-4">
            ...
            <div class="w-full bg-white rounded-xl shadow">
                ...
                $icon = collect(\App\Enums\AvailableSocialFieldsEnum::icons())->filter(fn($item, $social_key) =>
                str_starts_with($key, $social_key))?->first() ?? null;
