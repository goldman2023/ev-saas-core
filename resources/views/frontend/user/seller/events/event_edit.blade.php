...
@section('panel_content')
    ...
    <form class="" action="{{route('events.update', $event->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body p-0">
                        <ul class="nav nav-tabs nav-fill border-light">
                            @foreach (\App\Models\Language::all() as $key => $language)
                                <li class="nav-item">
                                    <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3" href="{{ route('seller.event.edit', ['id'=>$event->id, 'lang'=> $language->code] ) }}">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-lg-9">
                                <input type="text" class="form-control" name="title" placeholder="{{translate('Event title')}}" value="{{$event->getTranslation('title', $lang)}}" data-test="title">
                                @error('title')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-9">
                                <textarea class="form-control" name="description" data-test="description">{{$event->getTranslation('description', $lang)}}</textarea>
                                @error('description')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                                ...
                                <div class="col-md-8">
                                    @if ($attribute->type == 'dropdown')
                                        <select name="{{ $attribute->id }}" id="{{ 'dropdown_' . $attribute->id }}"
                                            ...
                                            data-live-search="true" data-placeholder="{{ translate('Choose Attributes') }}" data-test="{{ Str::slug($attribute->name, '-') }}">
                                            ...
                                            @foreach ($attribute->attribute_values as $option)
                                                <label class="aiz-checkbox">
                                                    <input type="checkbox" name="{{ $attribute->id }}[]"
                                                       ...
                                                       data-test="{{ Str::slug($attribute->name, '-') }}">
                                    ...
                                    @if ($attribute->type == 'number')
                                        ...
                                        @else
                                            <div class="input-group">
                                                <input type="number" placeholder="0.00" id="{{ 'plain_text_' . $attribute->id }}"
                                                    name="{{ $attribute->id }}" class="form-control" value="{{ $value }}"
                                                    step="0.01" data-test="{{ Str::slug($attribute->name, '-') }}">
                                                <div class="input-group-append">
