...
@section('panel_content')
    ...
    <div class="row gutters-10 justify-content-center">
        ...
        <div class="col-md-4 mx-auto mb-3" >
            <a href="{{ route('seller.event.create')}}">
              <div class="p-3 rounded mb-3 c-pointer text-center bg-white shadow-sm hov-shadow-lg has-transition">
                  <span class="size-60px rounded-circle mx-auto bg-secondary d-flex align-items-center justify-content-center mb-3">
                      <i class="las la-plus la-3x text-white"></i>
                  </span>
                  <div class="fs-18 text-primary">{{ translate('Add New Event') }}</div>
              </div>
            </a>
        </div>
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="md">{{ translate('Descripion')}}</th>
                ...
                <tbody>
                    @foreach ($events as $key => $event)
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td><a href="{{ route('event.visit', $event->slug) }}" target="_blank">{{ $event->getTranslation('title')}}</a></td>
                            <td>
                                <a href="{{ route('event.visit', $event->slug) }}" target="_blank" class="text-reset" data-test="detail-view">
                                    {{ $event->getTranslation('description') }}
                                </a>
                            </td>
                            <td><span><img style="width:50px;" src="{{URL::asset($event->upload->file_name)}}" alt="" srcset=""></span></td>
                            <td class="text-right">
                                <a href="{{route('seller.event.edit', ['id'=>$event->id, 'lang'=>config('app.locale')] )}}" class="btn btn-soft-info btn-icon btn-circle btn-sm" title="{{ translate('Edit') }}" data-test="editItem">
                                  <i class="las la-edit"></i>
                                </a>

                                <a href="javascript:void(0)" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('event.destroy', $event->id)}}" title="{{ translate('Delete') }}" data-test="deleteItem">
