		...
		<div class="col-md-4">
			<div class="bg-grad-1 text-white rounded-lg mb-4 overflow-hidden">
				<div class="px-3 pt-3 text-white">
                    @if(Session::has('cart'))
                        <div class="h3 fw-700 text-white">{{ count(Session::get('cart'))}} {{ translate('Product(s)') }}</div>
                    @else
                        <div class="h3 fw-700 text-white">0 {{ translate('Product') }}</div>
                    @endif
		...
		<div class="col-md-4">
			<div class="bg-grad-3 text-white rounded-lg mb-4 overflow-hidden">
				<div class="px-3 pt-3 text-white">
                    ...
                    @endphp
					<div class="h3 fw-700 text-white">{{ $total }} {{ translate('Product(s)') }}</div>
                ...
                <div class="card">
                    ...
                    <div class="card-body text-center">
                        ...
                        @if($customer_package != null)
                    		...
                    		<p class="mb-1 text-muted d-none">{{ translate('Product Upload') }}: {{ $customer_package->product_upload }} {{ translate('Times')}}</p>
                    		<p class="text-muted mb-4 d-none">{{ translate('Product Upload Remaining') }}: {{ auth()->user()->remaining_uploads }} {{ translate('Times')}}</p>
                            <h5 class="fw-600 mb-3 text-primary">{{ translate('Current Package') }}: {{ $customer_package->getTranslation('name') }}</h5>
                        @else
                            <h5 class="fw-600 mb-3 text-primary">{{translate('No Active Subsription')}}</h5>
                        @endif
