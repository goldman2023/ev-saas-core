  ...
  <div class="max-w-3xl mx-auto px-4 py-16 sm:px-6 sm:py-16 lg:px-8">
    <div class="w-full mb-3">
      ...
      @if($first_item->type === \App\Enums\ProductTypeEnum::bookable_service()->value)
        ...
        <p class="mt-2 text-base text-gray-500 mb-4">{{ translate('Your order #%d% has been processed. Please select the available booking time.') }}</p>
      ...
      @elseif($first_item->type === \App\Enums\ProductTypeEnum::standard()->value)
        ...
        <p class="mb-2 text-base text-gray-500">{{ translate('Your order #%d% has been received and will be processed asap.') }}</p>
      ...
      @if(!empty($first_item->getCoreMeta('unlockables')))
        <a href="{{ route('product.single.unlockable_content', ['slug' => $first_item->slug]) }}" class="btn-primary">
          {{ translate('See purchased content') }}
        </a>
      @endif
      
      @if(!empty($first_item->getCoreMeta('thank_you_cta_custom_url')) && !empty($first_item->getCoreMeta('thank_you_cta_custom_button_title')))
        <section class="py-4 md:py-6 mt-4 md:mt-6 border-t border-gray-200" style="background-image: url('flex-ui-assets/elements/pattern-white.svg'); background-position: center;">
          <div class="container px-4 mx-auto">
            <div class="flex flex-wrap items-center -mx-4">
              <div class="w-full md:w-1/2 px-4 mb-14 md:mb-0">
                <div class="max-w-md">
                  <h2 class="mb-2 text-18 md:text-20 font-heading font-bold">{{ $first_item->getCoreMeta('thank_you_cta_custom_title') }}</h2>
                  <p class="text-14 md:text-16 font-heading font-medium text-gray-600">{{ $first_item->getCoreMeta('thank_you_cta_custom_text') }}</p>
                </div>
              </div>
              <div class="w-full md:w-1/2 px-4">
                <div class="flex flex-wrap items-center md:justify-end">
                  <a class="btn-primary" href="{{ $first_item->getCoreMeta('thank_you_cta_custom_url') }}" target="_blank">
          ...
          @foreach($order->order_items as $item) --}}
            ...
            <div class="w-full flex flex-col">
              <div class="py-10 border-b border-gray-200 flex space-x-6">
                <img src="{{ $first_item->getThumbnail(['w' => 600]) }}" alt="" class="flex-none w-20 h-20 object-center object-contain bg-gray-100 rounded-lg sm:w-40 sm:h-40">
                <div class="flex-auto flex flex-col">
                  <div>
                      <h4 class="font-semibold text-gray-900">
                          <span>{{ $first_item->name }}</span>
                      </h4>
                      <p class="mt-2 text-sm text-gray-600 line-clamp-3">{{ $first_item->excerpt }}</p>
                  </div>
                  <div class="mt-6 flex-1 flex items-end">
                      <dl class="flex text-sm divide-x divide-gray-200 space-x-4 sm:space-x-6">
                          ...
                          <div class="pl-4 flex sm:pl-6">
                              ...
                              <dd class="ml-2 text-gray-700">{{ FX::formatPrice($first_item->total_price * $first_item->quantity) }}</dd>
                          </div>
                      </dl>
                  </div>
                </div>
