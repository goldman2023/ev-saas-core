<div class="grid grid-cols-10 gap-4">
    ...
    @php
      $selected_tag = 'section.data.'.$slot_name.'.components.'.$component_name.'.data.tag';
    @endphp
    <div class="col-span-3" x-data="{
        ...
        {{-- x-init="$watch('selectedTag', tag => $wire.set('{{ $selected_tag }}', tag))" --}}
        {{-- <input type="hidden" wire:model="{{ $selected_tag }}" /> --}}
    ...
    {{-- <div class="w-full mb-3">
        ...
        <div class="mt-1">
          <input type="text" wire:model.lazy="{{ $selected_tag }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
        </div>
    </div>
