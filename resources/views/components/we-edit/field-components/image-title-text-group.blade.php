              ...
              ">
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2 cursor-pointer" >
                  <div class="max-w-lg flex justify-center border-2 border-gray-300 border-dashed rounded-md"
                      :class="{'px-6 pt-5 pb-6': _.get(itt, 'image.file_name', '').length <= 0}"
                      x-on:click="$wire.emit('showMediaLibrary', id, 'image', [itt.image])">

                    <template x-if="_.get(itt, 'image.file_name', '').length > 0">
                      <div class="h-[200px] w-full rounded cursor-pointer">
                        <img class="w-full h-[200px] object-contain" x-bind:src="window.WE.IMG.url(_.get(itt, 'image.file_name', null))" />
                      </div>
                    </template>
