...
<div {{ $attributes->merge(['class' => 'relative bg-white border border-gray-200 rounded-lg py-5 px-4 lg:px-6']) }}>
    @if(!get_tenant_setting('support_phone'))
        ...
        <div class="absolute bottom-0 right-0 block h-full w-full bg-white/90 z-10">
            ...
            <!-- This example requires Tailwind CSS v2.0+ -->
            <a type="button" @can_access(['admin'], []) href="{{ route('settings.app_settings') }}" @else href="#" @endcan_access
                class="relative flex flex-col items-center justify-center w-full h-full border-2 border-gray-300 border-dashed rounded-lg p-4 text-center hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                <span class="emoji">
                    📱
                </span>
