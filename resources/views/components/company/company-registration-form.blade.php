...
<form id="shop" class="" action="{{ route('shops.store') }}" method="POST" enctype="multipart/form-data">
    ...
    @if (!Auth::check())
    <div class="fs-15 fw-600 p-3 border-bottom d-flex gap-20 align-items-center">

        <div class="flex-grow-2">
            {{ translate('Personal Info') }} -
        </div>

        <small class="flex-grow-1">{{ __('This information will be kept private at all times') }}
    ...
    <div class="p-3">
        <div class="form-group">
            ...
            <input type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}"
                value="{{ old('name') }}" value="{{ old('name') }}" placeholder="{{ translate('Name') }}" name="name"
                data-test="name">
            @error('name')
            <small class="text-danger">{{ $message }}</small>
            @enderror
    ...
    <div class="text-right">
        <button type="submit" class="btn btn-success btn-block btn-xl" style="border-radius:0; font-size: 20px;" data-test="submit">{{
            translate('Create Your Shop') }}</button>
    </div>
</form>
