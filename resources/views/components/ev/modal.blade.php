@if($hasTrigger)
    <!-- Modal Trigger -->
        <a class="btn btn-{{ $color }} transition-3d-hover {{ $btnClass }}" href="javascript:;"
           data-toggle="modal" data-target="#{{ $id }}"
        @if($triggerWireClick) wire:click="{!! $triggerWireClick !!}" @endif>{{ $btnText }}</a>
    <!-- End Modal Trigger -->
@endif


<!-- Add New Card Modal -->
<div class="modal @if(!$noAnimation) fade @endif" id="{{ $id }}" tabindex="-1" role="dialog" aria-hidden="true">
