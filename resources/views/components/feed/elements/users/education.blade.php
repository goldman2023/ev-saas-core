<div class="w-full bg-white rounded-xl shadow {{ $class }}">
    ...
    <div class="px-5 pb-4 flex flex-col ">
        ...
        <div
            class="w-full flex flex-col  @if($index !== count($user->getUserMeta('education')) - 1) border-b border-gray-200 pb-3 mb-3 @endif">
            ...
            <div class="flex flex-row items-center text-12 mt-1">
                @php
                $start_end_diff =
                \Carbon::createFromTimestamp($edu['end_date'])->diff(\Carbon::createFromTimestamp($edu['start_date']));
                $human_diff = $start_end_diff->y . ' years ' . $start_end_diff->d . ' days';
                @endphp
                <span class="text-typ-3">{{ \Carbon::createFromTimestamp($edu['start_date'])->format('M Y').' -
                    '.\Carbon::createFromTimestamp($edu['end_date'])->format('M Y') }}</span>
                <span class="text-10 text-typ-4 mx-1">•</span>
            ...
            <div class="w-full" x-data="{
                ...
                <p class="w-full mt-2 text-typ-3 text-14 overflow-hidden" :class="{'line-clamp-2':clamped}"
                    id="education_description_{{ $index }}">
                    {{ $edu['description'] }}
                </p>
                <span x-text="read_more_text"
                    class="w-full justify-center cursor-pointer text-right text-typ-3 text-12 mt-1" @click="readMore()"
                    :class="{'hidden':!read_more_visible, 'inline-flex':read_more_visible}"></span>
            </div>
            @endif
            ...
            <div class="w-full flex flex-row items-center mt-2">
                ...
                <a href="{{ Storage::url($cert['file_name']) }}" target="_blank"
                    class="bg-white flex items-center rounded-lg border border-gray-200 p-2">
        ...
        <div
            class="col-span-12 relative block w-full bg-white border-2 border-gray-300 border-dashed rounded-lg p-12 text-center">
